...
function renderRecipes(list) {
	document.getElementById('recipe-container').innerHTML = list.map(r => `
		<div class="recipe-card">
			<img src="${r.image}" alt="${r.name}">
			<div class="recipe-card-body">
				<span class="recipe-tag">${r.tags[0]}</span>
				<p class="recipe-title">${r.name}</p>
				<span class="rating" role="img" aria-label="Rating: ${r.rating} out of 5 stars">⭐⭐⭐⭐☆</span>
				<p>${r.description}</p>
			</div>
		</div>
	`).join('');
...
document.querySelector('button').addEventListener('click', () => {
	...
	const results = recipes
		.filter(r => r.name.toLowerCase().includes(query) || r.tags.some(t => t.toLowerCase().includes(query)))
