...
function renderHikes(list) {
  document.getElementById("hike-container").innerHTML = list.map(h => `
    <div class="hike-card">
      <div class="hike-content">
        <h2>${h.name}</h2>
        <div class="hike-tags">${h.tags.map(t => `<button>${t}</button>`).join("")}</div>
        <p>${h.description}</p>
        <p><span class="rating"">Difficulty: ${"🥾".repeat(h.difficulty)}${"▫️".repeat(5 - h.difficulty)}</span></p>
      </div>
    </div>
  `).join("");
}
...
document.querySelector("button").addEventListener("click", () => {
  ...
  const results = hikes
    .filter(h => h.name.toLowerCase().includes(query) || h.tags.some(t => t.toLowerCase().includes(query)))
